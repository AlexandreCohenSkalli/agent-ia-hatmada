...
export default function SuiviProspectionPage() {
  ...
  /** Auto-mark an email as replied (from SSE or poll) and sync to localStorage + stats */
    ...
    <div style={{ padding: '2rem' }}>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '0.5rem' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
          <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#1e293b' }}>Suivi — Prospection HATMADA</h1>
          {liveStatus !== 'off' && (
        ...
        <div style={{ display: 'flex', gap: '0.75rem' }}>
          <button onClick={() => { loadData(); pollReplies(); }}
            style={{ padding: '0.5rem 1rem', background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: '0.5rem', fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer' }}>
            ↻ Actualiser
          </button>
          <button onClick={() => {
            if (!confirm('Réinitialiser tout le suivi Prospection ? Cette action est irréversible.')) return;
      ...
      {lastReplyAt && (
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#f0fdf4', border: '1px solid #86efac', borderRadius: '0.625rem', padding: '0.75rem 1rem', marginBottom: '1rem' }}>
          <span style={{ color: '#15803d', fontWeight: 600, fontSize: '0.9375rem' }}>
            🎉 Nouvelle réponse reçue ! Mise à jour automatique effectuée.
          </span>
      ...
      <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
        ...
        {(['all', 'sent', 'replied'] as const).map(f => (
          <button key={f} onClick={() => setFilter(f)}
            style={{ padding: '0.5rem 1.125rem', borderRadius: '0.5rem', fontWeight: 600, fontSize: '0.875rem', cursor: 'pointer', border: filter === f ? 'none' : '1px solid #e2e8f0', background: filter === f ? 'linear-gradient(to right, #2563eb, #06b6d4)' : 'white', color: filter === f ? 'white' : '#475569' }}>
              ...
              <div key={email.id} style={{ background: 'white', borderRadius: '0.75rem', padding: '1rem 1.25rem', boxShadow: '0 2px 8px rgba(0,0,0,0.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexShrink: 0 }}>
                  ...
                  <button onClick={() => setSelected(email)}
                    style={{ padding: '0.375rem 0.875rem', background: '#eff6ff', color: '#2563eb', border: '1px solid #bfdbfe', borderRadius: '0.375rem', fontWeight: 600, fontSize: '0.8125rem', cursor: 'pointer', whiteSpace: 'nowrap' }}>
                    Voir l'email
                  </button>
                </div>
              </div>
