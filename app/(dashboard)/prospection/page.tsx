...
export default function ProspectionPage() {
  ...
  return (
    <div style={{ padding: '2rem' }}>
      <h1 style={{ fontSize: '1.75rem', fontWeight: 700, color: '#1e293b', marginBottom: '1.5rem' }}>Prospection HATMADA</h1>
      ...
      {emails.length > 0 && (
        <div style={{ display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap', alignItems: 'center' }}>
          {[
            ...
            { label: 'Envoyés', value: emails.filter(e => e.status === 'sent').length, color: '#16a34a' },
          ...
          <div style={{ background: 'white', borderRadius: '1rem', padding: '2rem', width: '100%', maxWidth: '640px', maxHeight: '90vh', overflowY: 'auto', boxShadow: '0 25px 60px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ display: 'flex', gap: '1rem' }}>
              <button onClick={() => { handleSendEmail(selectedEmail); setShowPreview(false); }} disabled={sendingEmail || selectedEmail.status !== 'pending'}
                style={{ flex: 1, padding: '0.875rem', background: selectedEmail.status !== 'pending' ? '#cbd5e1' : 'linear-gradient(to right, #2563eb, #06b6d4)', color: 'white', border: 'none', borderRadius: '0.5rem', fontWeight: 600, cursor: selectedEmail.status !== 'pending' ? 'not-allowed' : 'pointer' }}>
      ...
      {emails.length === 0 && !uploading && (
        <div style={{ background: 'white', borderRadius: '0.875rem', padding: '3rem', textAlign: 'center', boxShadow: '0 2px 8px rgba(0,0,0,0.07)' }}>
          ...
          <p style={{ color: '#94a3b8', fontSize: '1rem' }}>Importez un fichier XLSX pour générer les emails</p>
        </div>
      )}
    </div>
  );
